...
exports.default = exports.ENDPOINT_FROM_PROTOBUF_REGISTRY = exports.ENDPOINT_FROM_JSON_REGISTRY = void 0;
 ...
 */
const ENDPOINT_FROM_PROTOBUF_REGISTRY = exports.ENDPOINT_FROM_PROTOBUF_REGISTRY = new Map();
 ...
 */
const ENDPOINT_FROM_JSON_REGISTRY = exports.ENDPOINT_FROM_JSON_REGISTRY = new Map();
...
class RegisteredServiceEndpoint {
  ...
  get type() {
    if (this._type == null) {
      throw new Error("RegisteredServiceEndpoint type is not set; use a concrete endpoint subtype.");
    }
    return this._type;
  }
  ...
  _applyJsonBaseFields(json) {
    if (json.ip_address != null) {
      ...
      throw new Error("Registered service endpoint response did not include an IP address or domain name.");
    }
  ...
  static _fromProtobuf(endpoint) {
    const endpointType = RegisteredServiceEndpoint._getEndpointType(endpoint);
    const factory = endpointType != null ? ENDPOINT_FROM_PROTOBUF_REGISTRY.get(endpointType) : null;
    if (factory == null) {
      throw new Error("Unable to decode registered service endpoint: endpoint type is missing.");
    }
  ...
  static _fromJson(json) {
    const endpointType = RegisteredServiceEndpoint._getJsonEndpointType(json);
    const factory = endpointType != null ? ENDPOINT_FROM_JSON_REGISTRY.get(endpointType) : null;
    if (factory == null) {
      throw new Error("Registered service endpoint response did not include a recognized endpoint type.");
    }
    return factory(json);
  }
  ...
  static _getEndpointType(endpoint) {
    const endpointWithOneOf = /** @type {{ endpointType?: RegisteredServiceEndpointType }} */
    endpoint;
  ...
  static _getJsonEndpointType(json) {
    if (json.type != null) {
      const normalized = json.type.replace(/[^A-Za-z0-9]/g, "").toUpperCase();
      switch (normalized) {
        case "BLOCKNODE":
          return "blockNode";
        case "MIRRORNODE":
 ...
 * @returns {Uint8Array}
 */
exports.default = RegisteredServiceEndpoint;
function parseIpAddress(ipAddress) {
  return ipAddress.includes(":") ? parseIpv6Address(ipAddress) : parseIpv4Address(ipAddress);
}
...
function parseIpv4Address(ipAddress) {
  ...
  return Uint8Array.from(octets.map(octet => {
    if (!/^\d+$/.test(octet)) {
      throw new Error(`Invalid IPv4 address: ${ipAddress}`);
    }
...
function parseIpv6Address(ipAddress) {
  const normalizedAddress = ipAddress.startsWith("[") && ipAddress.endsWith("]") ? ipAddress.slice(1, -1) : ipAddress;
  ...
  const zeroFillSize = parts.length === 2 ? 8 - (left.length + right.length) : 0;
  if (zeroFillSize < 0 || parts.length === 1 && left.length !== 8 || parts.length === 2 && zeroFillSize === 0) {
    throw new Error(`Invalid IPv6 address: ${ipAddress}`);
  }
