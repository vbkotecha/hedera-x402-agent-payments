...
class BlockNodeApi {
  ...
  static _fromCode(code) {
    ...
    throw new Error(`(BUG) BlockNodeApi._fromCode() does not handle code: ${code}`);
  }
  ...
  static _fromString(name) {
    ...
    switch (normalized) {
      ...
      case "STATEPROOF":
        return BlockNodeApi.StateProof;
      case "UNRECOGNIZED":
        throw new Error("Mirror node returned an unrecognized block node API.");
      default:
        throw new Error(`Unsupported block node API returned by mirror node: ${name}`);
    }
  }
