...
function isRetryableNetworkError(err) {
  ...
  return /timeout|timed out|network|fetch failed|ECONN|ENETUNREACH/i.test(message);
}
...
async function readErrorDetail(response) {
  try {
    ...
    try {
      ...
      const parsed = /** @type {{_status?: {messages?: {message?: string, detail?: string}[]}}} */
      JSON.parse(text);
      ...
      if (typeof detail === "string" && detail.length > 0) {
        return typeof message === "string" && message.length > 0 ? `${message}: ${detail}` : detail;
      }
