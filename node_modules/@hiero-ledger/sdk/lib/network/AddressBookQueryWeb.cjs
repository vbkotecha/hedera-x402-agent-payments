    ...
    while (!isLastPage) {
      ...
      for (let attempt = 0; attempt <= maxAttempts; attempt++) {
        try {
          ...
          if (attempt < maxAttempts && !client.isClientShutDown && this._retryHandler(/** @type {MirrorError | Error | null} */error)) {
          ...
          const errorMessage = maxAttemptsReached ? `Failed to query address book after ${maxAttempts + 1} attempts. Last error: ${message}` : `Failed to query address book: ${message}`;
