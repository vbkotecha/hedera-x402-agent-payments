...
var _mirrorRestRetry = require("./mirrorRestRetry.cjs");
...
class RegisteredNodeAddressBookQuery {
  ...
  async _makeFetchRequest(client, resolve, reject, requestTimeout) {
    ...
    while (!isLastPage) {
      ...
      try {
        const data = await this._fetchPage(currentUrl, maxAttempts, maxBackoff, requestTimeout);
        if (data.registered_nodes != null) {
          for (const entry of data.registered_nodes) {
            aggregatedNodes.push(_RegisteredNode.default._fromJson(entry));
          }
        }
    ...
    resolve(new _RegisteredNodeAddressBook.default({
    ...
    for (let attempt = 1; attempt <= totalAttempts; attempt++) {
      try {
        ...
        if (response.ok) {
          ...
          await response.json();
          return /** @type {RegisteredNodeAddressBookQueryResponse} */responseJson;
        }
        ...
        if (attempt < totalAttempts && (0, _mirrorRestRetry.isRetryableNetworkError)(err)) {
          this._logRetry(attempt, backoff, lastError.message);
          await sleep(backoff);
          backoff = Math.min(backoff * 2, maxBackoff);
          continue;
  ...
  _logRetry(attempt, backoff, message) {
    if (this._logger) {
      this._logger.debug(`Error getting registered nodes from mirror node during attempt ${attempt}. Waiting ${backoff} ms before next attempt: ${message}`);
    }
  }
}
 ...
 * @returns {Promise<void>}
 */
exports.default = RegisteredNodeAddressBookQuery;
function sleep(ms) {
