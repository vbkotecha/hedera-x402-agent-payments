...
class FeeEstimateResponse {
  ...
  constructor(props) {
     ...
     * @readonly
     */
    this.highVolumeMultiplier = _long.default.fromValue(props.highVolumeMultiplier != null ? props.highVolumeMultiplier : 1);
     ...
     * @readonly
     */
    this.total = _long.default.fromValue(props.total);
  }
  ...
  static _fromJSON(response) {
    return new FeeEstimateResponse({
      highVolumeMultiplier: response.high_volume_multiplier != null ? response.high_volume_multiplier : 1,
      networkFee: response.network ? _NetworkFee.default._fromJSON(response.network) : new _NetworkFee.default({
        multiplier: 0,
        subtotal: 0
      }),
      nodeFee: response.node ? _FeeEstimate.default._fromJSON(response.node) : new _FeeEstimate.default({
        base: 0,
        extras: []
      }),
      serviceFee: response.service ? _FeeEstimate.default._fromJSON(response.service) : new _FeeEstimate.default({
        base: 0,
        extras: []
      }),
...
exports.default = FeeEstimateResponse;