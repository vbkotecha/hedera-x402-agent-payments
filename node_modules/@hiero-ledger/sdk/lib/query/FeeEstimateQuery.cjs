...
var _FeeEstimateMode = _interopRequireDefault(require("./enums/FeeEstimateMode.cjs"));
var _FeeEstimateResponse = _interopRequireDefault(require("./FeeEstimateResponse.cjs"));
var _NetworkFee = _interopRequireDefault(require("./NetworkFee.cjs"));
var _FeeEstimate = _interopRequireDefault(require("./FeeEstimate.cjs"));
var HieroProto = _interopRequireWildcard(require("@hiero-ledger/proto"));
var _mirrorRestRetry = require("../network/mirrorRestRetry.cjs");
...
class FeeEstimateQuery {
  ...
  constructor(props = {}) {
     ...
     * @type {number}
     */
    this._mode = _FeeEstimateMode.default.INTRINSIC;
  ...
  setMode(mode) {
    ...
    const validValues = Object.values(_FeeEstimateMode.default).map(Number);
    if (!validValues.includes(modeValue)) {
      const validModes = Object.entries(_FeeEstimateMode.default).map(([key, value]) => `${key} (${Number(value)})`).join(", ");
      throw new Error(`Invalid FeeEstimateMode: ${modeValue}. Must be one of: ${validModes}`);
    }
  ...
  setHighVolumeThrottle(throttle) {
    ...
    if (!Number.isInteger(value) || value < 0 || value > HIGH_VOLUME_THROTTLE_MAX_BPS) {
      throw new Error(`Invalid highVolumeThrottle: ${throttle}. Must be an integer in [0, ${HIGH_VOLUME_THROTTLE_MAX_BPS}].`);
    }
  ...
  _makeMirrorNodeRequest(client, resolve, reject) {
    ...
    for (let chunk = 0; chunk < chunks; chunk++) {
      ...
      perChunkPromises.push(this._requestFeeEstimateForIndex(client, txObj, index));
    }
    Promise.all(perChunkPromises).then(responses => {
  ...
  _aggregateFeeResponses(responses) {
    if (responses.length === 0) {
      return new _FeeEstimateResponse.default({
        highVolumeMultiplier: 1,
        networkFee: new _NetworkFee.default({
          multiplier: 0,
          subtotal: 0
        }),
        nodeFee: new _FeeEstimate.default({
          base: 0,
          extras: []
        }),
        serviceFee: new _FeeEstimate.default({
          base: 0,
          extras: []
        }),
        total: 0
    ...
    const nodeExtrasSubtotal = nodeExtras.reduce((sum, extra) => sum + Number(extra.subtotal), 0);
    const serviceExtrasSubtotal = serviceExtras.reduce((sum, extra) => sum + Number(extra.subtotal), 0);
    ...
    const total = networkSubtotal + aggregatedNodeTotal + aggregatedServiceTotal;
    return new _FeeEstimateResponse.default({
  ...
  _buildRequestUrl(client) {
    ...
    params.set("mode", this._mode === _FeeEstimateMode.default.STATE ? "STATE" : "INTRINSIC");
    if (this._highVolumeThrottle > 0) {
      params.set("high_volume_throttle", String(this._highVolumeThrottle));
    }
  ...
  _requestFeeEstimateForIndex(client, txObj, index) {
    return new Promise((res, rej) => {
      ...
      const tx = /** @type {HieroProto.proto.ITransaction | undefined} */
      ...
      this._fetchWithRetry(url, buffer).then(data => res(_FeeEstimateResponse.default._fromJSON(data))).catch(error => {
  ...
  async _fetchWithRetry(url, body) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        const errorDetail = await (0, _mirrorRestRetry.readErrorDetail)(response);
        if (response.status === 400) {
          // 400 = malformed transaction (INVALID_ARGUMENT). Do not retry.
          throw new Error(`HTTP 400 Bad Request${errorDetail ? `: ${errorDetail}` : ""}`);
        }
        if (response.status === 500 || response.status === 503 || response.status === 504) {
          lastError = new Error(`HTTP ${response.status}${errorDetail ? `: ${errorDetail}` : ""}`);
        ...
        if (attempt < MAX_ATTEMPTS && (0, _mirrorRestRetry.isRetryableNetworkError)(err)) {
          await sleep(backoff);
          backoff = Math.min(backoff * 2, MAX_BACKOFF_MS);
          continue;
        }
 ...
 */
exports.default = FeeEstimateQuery;
function sleep(ms) {
