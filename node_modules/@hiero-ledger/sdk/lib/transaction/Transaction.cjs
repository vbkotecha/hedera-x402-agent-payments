...
class Transaction extends _Executable.default {
   ...
   */
  estimateFee() {
    return new _FeeEstimateQuery.default({
      transaction: this
    });
  }
  ...
  static _validateTransactionBodies(transactionDataCase, transactionIds, nodeIds, bodies) {
    ...
    if (isChunkedTransactionType) {
      Transaction._validateChunkedTransactionBodies(transactionDataCase, bodies, groupSize);
    }
  }
   ...
   * @returns {void}
   */
  static _validateChunkedTransactionBodies(transactionDataCase, bodies, groupSize) {
    const ignoredFields = Transaction._getChunkedTransactionIgnoredFields(transactionDataCase);
    const referenceBody = bodies[0];
    for (let bodyIndex = groupSize; bodyIndex < bodies.length; bodyIndex += groupSize) {
      if (!util.compare(referenceBody, bodies[bodyIndex], ignoredFields)) {
        throw new Error("failed to validate transaction bodies");
      }
    }
  }
