...
class TransferTransaction extends _AbstractTokenTransferTransaction.default {
  ...
  addHbarTransferWithHook(accountId, amount, hook) {
    ...
    return this._addHbarTransfer(accountId, amount, isApproved, new _FungibleHookCall.default({
      hookId: hook.hookId,
  ...
  addNftTransferWithHook(nftId, sender, receiver, senderHookCall, receiverHookCall) {
    return this._addNftTransfer(false, nftId, sender, receiver, undefined,
    // receiver
  ...
  addTokenTransferWithHook(tokenId, accountId, amount, hook) {
    const fungibleHook = new _FungibleHookCall.default({
      hookId: hook.hookId != null ? hook.hookId : undefined,
      evmHookCall: hook.evmHookCall != null ? hook.evmHookCall : undefined,
      type: hook.type
    });
    const isApproved = false; // this is not approved transfer, adding comment for clarity
    const expectedDecimals = null; // we don't expect decimals here, adding comment for clarity

    return this._addTokenTransfer(tokenId, accountId, amount, isApproved, expectedDecimals, fungibleHook);
  }
  ...
  _makeTransactionData() {
    ...
    return {
      transfers: {
        accountAmounts: this._hbarTransfers.map(transfer => transfer._toProtobuf())
      },
      tokenTransfers
    };
  }
