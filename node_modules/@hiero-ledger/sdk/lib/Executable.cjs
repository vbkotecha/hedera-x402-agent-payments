...
class Executable {
  ...
  _shouldRetryRequestError(error, attempt) {
    return (error instanceof _GrpcServiceError.default || error instanceof _HttpError.default) && this._shouldRetryExceptionally(error) && attempt <= (/** @type {number} */this._maxAttempts);
  }
  ...
  _getExecutionNode(client) {
    ...
    if (currentNode == null) {
      throw new Error(`NodeAccountId not recognized: ${this._nodeAccountIds.current.toString()}`);
    }
    return currentNode;
  }
  ...
  async _handleUnhealthyNode(currentNode, request, attempt, isLocalNode) {
    ...
    if (isSingleNodeReceiptOrRecordRequest || isLocalNode) {
      await delayForAttempt(isLocalNode, attempt, /** @type {number} */this._minBackoff, this._maxBackoff);
      return;
    }
    ...
    this._logger?.debug(`[${this._getLogId()}] Node is not healthy, trying the next node.`);
  ...
  async _handleInvalidNodeAccountId(client, currentNode, nodeAccountId) {
    this._logger?.debug(`[${this._getLogId()}] node with accountId: ${nodeAccountId.toString()} and proxy IP: ${currentNode.address.toString()} has invalid node account ID, marking as unhealthy and updating network`);
    ...
    try {
      if (client.mirrorNetwork.length > 0) {
        ...
        this._logger?.warn(`[${this._getLogId()}] Cannot update address book: no mirror network configured. Retrying with existing network configuration.`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      this._logger?.trace(`[${this._getLogId()}] failed to update client address book after INVALID_NODE_ACCOUNT_ID: ${errorMessage}`);
    }
  }
  ...
  async _executeRequestWithGrpcDeadline(channel, request) {
    ...
    if (this._grpcDeadline != null) {
      promises.push(new Promise((_, reject) => {
        deadlineTimer = setTimeout(() => reject(new _GrpcServiceError.default(_GrpcStatus.default.DeadlineExceeded)), /** @type {number=} */this._grpcDeadline);
      }));
    }
  ...
  async execute(client, requestTimeout) {
    ...
    // The retry loop
    for (let attempt = 1; attempt <= (/** @type {number} */this._maxAttempts); attempt += 1) {
      if (this._requestTimeout != null && requestStartTime + this._requestTimeout <= Date.now()) {
      ...
      if (this._shouldSkipAttemptForNodeAccountId(this._nodeAccountIds.current)) {
        console.error(`Attempting to execute a transaction against node ${this._nodeAccountIds.current.toString()}, which is not included in the Client's node list. Please review your Client configuration.`);
        this._nodeAccountIds.advance();
        continue;
      }
      const executionNode = this._getExecutionNode(client);
      this._logger?.debug(`[${this._getLogId()}] Node AccountID: ${executionNode.accountId.toString()}, IP: ${executionNode.address.toString()}`);
      ...
      if (!executionNode.isHealthy()) {
        await this._handleUnhealthyNode(executionNode, request, attempt, isLocalNode);
        continue;
      }
      this._nodeAccountIds.advance();
      let response;
      try {
        response = await this._executeRequestWithGrpcDeadline(channel, request);
      } catch (err) {
        ...
        this._logger?.debug(`[${this._getLogId()}] received error ${JSON.stringify(error)}`);
        ...
        if (this._shouldRetryRequestError(error, attempt)) {
          ...
          this._logger?.debug(`[${this._getLogId()}] node with accountId: ${executionNode.accountId.toString()} and proxy IP: ${executionNode.address.toString()} is unhealthy`);
          if (executionNode.isHealthy()) {
      ...
      const [status, shouldRetry] = this._getStatusAndExecutionState(request, response);
      const isError = status.toString() !== _Status.default.Ok.toString() && status.toString() !== _Status.default.Success.toString();
      if (isError) {
      ...
      switch (shouldRetry) {
        case ExecutionState.Retry:
          if (status === _Status.default.InvalidNodeAccount) {
            await this._handleInvalidNodeAccountId(client, executionNode, executionNode.accountId);
          }
          await delayForAttempt(isLocalNode, attempt, /** @type {number} */this._minBackoff, this._maxBackoff);
          continue;
        case ExecutionState.Finished:
          return this._mapResponse(response, executionNode.accountId, request);
        case ExecutionState.Error:
          throw this._mapStatusError(request, response, executionNode.accountId);
        default:
    ...
    throw new _MaxAttemptsOrTimeoutError.default(`max attempts of ${/** @type {number} */this._maxAttempts.toString()} was reached for request with last error being: ${persistentError != null ? persistentError.toString() : ""}`, this._nodeAccountIds.current.toString());
  }
