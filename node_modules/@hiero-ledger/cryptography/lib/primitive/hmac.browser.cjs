...
async function hash(algorithm, secretKey, data) {
  ...
  try {
    const key_ = await window.crypto.subtle.importKey("raw", (0, _utils.toBufferSource)(key), {
      name: "HMAC",
      hash: algorithm
    }, false, ["sign"]);
    return new Uint8Array(await window.crypto.subtle.sign("HMAC", key_, (0, _utils.toBufferSource)(value)));
  } catch {
