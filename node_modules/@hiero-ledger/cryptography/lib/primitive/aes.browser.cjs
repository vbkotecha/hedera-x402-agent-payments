...
async function createCipheriv(algorithm, key, iv, data) {
  ...
  switch (algorithm.toUpperCase()) {
    case CipherAlgorithm.Aes128Ctr:
      algorithm_ = {
        name: "AES-CTR",
        counter: (0, _utils.toBufferSource)(iv),
        length: 128
      };
      break;
    case CipherAlgorithm.Aes128Cbc:
      algorithm_ = {
        name: "AES-CBC",
        iv: (0, _utils.toBufferSource)(iv)
      };
      break;
    default:
      throw new Error("(BUG) non-exhaustive switch statement for CipherAlgorithm");
  }
  const key_ = await window.crypto.subtle.importKey("raw", (0, _utils.toBufferSource)(key), algorithm_.name, false, ["encrypt"]);
  return new Uint8Array(
  // https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto/encrypt#return_value
  /** @type {ArrayBuffer} */
  await window.crypto.subtle.encrypt(algorithm_, key_, (0, _utils.toBufferSource)(data)));
}
...
async function createDecipheriv(algorithm, key, iv, data) {
  ...
  const key_ = await window.crypto.subtle.importKey("raw", (0, _utils.toBufferSource)(key), algorithm_.name, false, ["decrypt"]);
  let decrypted;
  try {
    decrypted = await window.crypto.subtle.decrypt(algorithm_, key_, (0, _utils.toBufferSource)(data));
  } catch (error) {
    const message = error != null && /** @type {Error} */error.message != null ? /** @type {Error} */error.message : "";
